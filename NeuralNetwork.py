...
def benchmark(X, z, parameters, NeuralNetwork=None, mode=None, writingPermissions=None):
...
class NN():
    ...
    def predict(self, X=None):
        ...
        if X is not None:
            ...
            z_storage = self.z
            self.X = X
            self.z = np.zeros(X.shape[0], self.Ncategories)
    ...
    def train(self):
        ...
        Splits dataset into training and testing data (80%20%, respectively)

        Only trains on batches of size self.minibatchSize from training data at
        a time

        After training, the network is fed one last time with test data to 
        generate a score
        """
        ...
        self.scoreHistory = np.zeros((epochN, 2))
